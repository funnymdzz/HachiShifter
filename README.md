...
- 轨道菜单可新建旋律/普通音轨并重命名；编辑菜单支持采样复制、粘贴和深度复制，
  复制后的音符使用独立 ID 且只保留采样内部连接；
...
- 音符整体移调并保留原始音高调制；
- 编辑菜单支持对多选音符按音分移调、设置统一 MIDI 音高、求平均音高及量化到半音，
  所有操作均整体移动原始幅度形状而非拉平音高线；
- 多选音符可复制、剪切并在播放位置粘贴到目标采样；粘贴生成独立音符 ID，保留
  音高线、辅音、齿音及内部连接，同时限制在目标采样的素材范围内；
...
- 旋律钢琴卷帘按当前轨道显示，重叠音符自动分配紧凑子层，避免多轨或同音高素材
  堆叠遮挡；支持框选、Shift/Ctrl 多选、Alt 精细音分移动、网格吸附时长手柄、
  双击音符量化、双击辅音边界对齐，以及连接工具内双击分割音符；已标注的发音/
  别名可在设置中选择显示；
- 原始采样扳手编辑模式；
...
- `.hjpx` 原生工程保存与读取（兼容打开旧 `.hspx`）、撤销/重做及 WAV 混音导出；工程同时保存
  素材绝对/相对路径，整个工程目录移动后会自动按相对路径和文件名重定位；
- 文件菜单支持新建、保存、另存为和最近八个工程；关闭、切换或新建工程前会检查
  未保存修改，并可选择保存、放弃修改或取消；
- mld5 周期/非周期分离、共振峰包络保持、Attack 保护和块能量归一化；
- 直接集成 libllsm2 的两层语音模型作为 `llsm2` 变调/拉伸选项，仅接入其
  harmonic+noise、声门源/声道包络分析合成算法，不构建 moresampler 前端；
- NSF-HiFiGAN 直接从原始 PCM 映射 Mel，避免预拉伸改变共振峰；立体声输入采用
  相位相关的单声道选择，模型输入增加边缘上下文并修复孤立脉冲以抑制爆音。
  切换到 NSF-HiFiGAN 时，拉伸菜单会额外显示“可变 Hop Mel 先拼接后合成”专属模式；
  稳健音高线开关仅在 mld5 下显示；
