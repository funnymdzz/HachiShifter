...
pub(crate) fn build_clip_input_pitch_curve(
    ...
    let timeline_midi_raw: Vec<f32> = if is_vslib {
        Vec::new()
    } else if !clip.melodyne_warp_segments.is_empty() {
        // MPD imports already provide the authoritative original F0 contour in
        // destination time. Re-analysing the source and applying one average
        // playback_rate de-synchronizes F0 from Melodyne's piecewise attack /
